...
export default function MY_RP_000M() {
  ...
  const [errCdList, setErrCdList] = useState<TObj>({});
  const errorStatCds = ["99", "91", "92", "93", "94"];

  /** 수령용 계좌 조회 */
  const { data: rpayAcctList, isLoading: rpayAcctListLoading } =
    useQuery<TSearchRpyAcctRes>({
  ...
  const {
    ...
    data,
    isPending: payRefundRequestPending,
  } = useMutation({
    mutationKey: ["pay-refund-req"],
    mutationFn: (body: { bankCd: string; payAmt: number; gpsInfo: string }) =>
      fetchApi({
   ...
   * 챕터 1 타이틀? 죽음?
   *
   * 영화 <폰 부스>를 보는 와중, 눈물이 흘러 내렸다.
   * */
  const isLocationLoaded = location.loaded;
  /**
   * 이종족이 주인공인 컨셉 > 드워프? 엘프? 장생족? 영생족? 불멸자? 필멸자? 오크? 오우거? 트롤?
   * : 월드 오브 워크래프트 종족 참고
   *
   * 프로그래밍 오류로 인해 자아를 갖게 된 NPC의 이야기?
   * - #이종족 #NPC #유저들의대화못알아들음 #장비착용불가 #레벨업불가
   * */
  ...
  useEffect(() => {
    if (Array.isArray(repayList)) {
      // statCdList 업데이트
      const newStatCdList = Object.assign(
        {},
        ...repayList.map((el) => ({ [el.bankCd]: el.statCd }))
      );
      setStatCdList(newStatCdList);

      // errCdList 업데이트 (statCd가 실패 코드이고 statNm이 string이고 지급실패[코드] 형식일 때만)
      const newErrCdList = Object.assign(
        {},
        ...repayList.map((el: any) => {
          if (
            errorStatCds.includes(el.statCd) &&
            typeof el.statNm === "string" &&
            el.statNm !== null &&
            !!el.statNm
          ) {
            const match = el.statNm.match(/^지급실패\[(.+)\]$/);
            return match?.[1] ? { [el.statNm]: match[1] } : {};
          }
          return {};
        })
      );
      setErrCdList(newErrCdList);
    }
  ...
  return (
    <>
      {(payRefundRequestPending || rpayAcctListLoading) && <Loading />}
      <div className="pt-6">
